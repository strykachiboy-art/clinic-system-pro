...
class StaffUser(HttpUser):
    ...
    def list_staff(self) -> None:
        ...
        with self.client.get(
            ...
            headers=headers,
            name="GET /api/v1/staff [page=1,per_page=50]",
            catch_response=True,
        ) as response:
            if response.status_code != 200:
                response.failure(
            ...
            if not isinstance(body, dict):
                response.failure(
                    "Staff response is not an object"
                )
                return
            ...
            if len(items) > 50:
                response.failure(
                    f"Returned more than 50 staff records: {len(items)}"
                )
                return
            ...
            for item in items:
                ...
                if not isinstance(item.get("id"), int):
                    response.failure(
                        "Staff id is not an integer"
                    )
                    return

                if not isinstance(item.get("clinic_id"), int):
                    response.failure(
                        "Staff clinic_id is not an integer"
                    )
                    return

                if not isinstance(item.get("user_id"), int):
                    response.failure(
                        "Staff user_id is not an integer"
                    )
                    return

                if not isinstance(item.get("first_name"), str):
                    response.failure(
                        "Staff first_name is not a string"
                    )
                    return

                if not isinstance(item.get("last_name"), str):
                    response.failure(
                        "Staff last_name is not a string"
                    )
                    return

                if not isinstance(item.get("status"), str):
                    response.failure(
                        "Staff status is not a string"
                    )
                    return
