...
class InventoryUser(HttpUser):
    ...
    def list_inventory_items(self) -> None:
        ...
        with self.client.get(
            ...
            headers=headers,
            name="GET /api/v1/inventory/items [page=1,per_page=50]",
            catch_response=True,
        ) as response:
            if response.status_code != 200:
                response.failure(
            ...
            if not isinstance(data, dict):
                response.failure(
                    "Inventory data is missing or invalid"
                )
                return
            ...
            if not isinstance(items, list):
                response.failure(
                    "Inventory items is not a list"
                )
                return

            if not isinstance(total, int):
                response.failure(
                    "Inventory total is not an integer"
                )
                return
            ...
            if len(items) > 50:
                response.failure(
                    f"Returned more than 50 inventory items: "
                    f"{len(items)}"
                )
                return
            ...
            for item in items:
                if not isinstance(item, dict):
                    response.failure(
                        "Inventory item is not an object"
                    )
                    return

                if not isinstance(item.get("id"), int):
                    response.failure(
                        "Inventory item id is not an integer"
                    )
                    return

                if not isinstance(item.get("name"), str):
                    response.failure(
                        "Inventory item name is not a string"
                    )
                    return

                if not isinstance(item.get("quantity_on_hand"), int):
                    response.failure(
                        "Inventory quantity_on_hand is not an integer"
                    )
                    return
