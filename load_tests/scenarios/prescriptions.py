...
class PrescriptionsUser(HttpUser):
    wait_time = between(
    ...
    @task
    def list_patient_prescriptions(self) -> None:
        patient_id = int(
            os.getenv(
        ...
        if patient_id <= 0:
            raise RuntimeError(
                "LOCUST_PRESCRIPTION_PATIENT_ID must be positive"
            )
        ...
        with self.client.get(
            f"/api/v1/prescriptions/patients/"
            f"{patient_id}?page=1&per_page=50",
            headers=headers,
            name=(
                "GET /api/v1/prescriptions/patients/{patient_id} "
                "[page=1,per_page=50]"
            ),
            catch_response=True,
        ) as response:
            ...
            if not isinstance(data, dict):
                response.failure(
                    "Prescription data is missing or invalid"
                )
                return
            ...
            if not isinstance(items, list):
                response.failure(
                    "Prescription items is not a list"
                )
                return

            if not isinstance(total, int):
                response.failure(
                    "Prescription total is not an integer"
                )
                return
            ...
            if len(items) > 50:
                response.failure(
                    f"Returned more than 50 prescriptions: "
                    f"{len(items)}"
                )
                return
            ...
            for item in items:
                if not isinstance(item, dict):
                    response.failure(
                        "Prescription item is not an object"
                    )
                    return

                if not isinstance(item.get("id"), int):
                    response.failure(
                        "Prescription id is not an integer"
                    )
                    return

                if item.get("patient_id") != patient_id:
                    response.failure(
                        "Prescription belongs to unexpected patient"
                    )
                    return

                if not isinstance(item.get("items"), list):
                    response.failure(
                        "Prescription medication items is not a list"
                    )
                    return
