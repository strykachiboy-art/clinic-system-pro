...
class PharmacyUser(HttpUser):
    ...
    def list_drugs(self) -> None:
        ...
        with self.client.get(
            ...
            headers=headers,
            name="GET /api/v1/pharmacy/drugs [page=1,per_page=50]",
            catch_response=True,
        ) as response:
            if response.status_code != 200:
                response.failure(
            ...
            if not isinstance(data, dict):
                response.failure(
                    "Pharmacy drug data is missing or invalid"
                )
                return
            ...
            if not isinstance(items, list):
                response.failure(
                    "Drug items is not a list"
                )
                return

            if not isinstance(total, int):
                response.failure(
                    "Drug total is not an integer"
                )
                return
            ...
            if len(items) > 50:
                response.failure(
                    f"Returned more than 50 drugs: {len(items)}"
                )
                return
            ...
            for item in items:
                if not isinstance(item, dict):
                    response.failure(
                        "Drug item is not an object"
                    )
                    return

                if not isinstance(item.get("id"), int):
                    response.failure(
                        "Drug id is not an integer"
                    )
                    return

                if not isinstance(item.get("name"), str):
                    response.failure(
                        "Drug name is not a string"
                    )
                    return
