...
class AppointmentsUser(HttpUser):
    wait_time = between(
    ...
    @task
    def staff_appointment_schedule(self) -> None:
        headers = {
            "Accept": "application/json",
        ...
        with self.client.get(
            "/api/v1/appointments/staff/20?page=1&per_page=50",
            headers=headers,
            name="GET /api/v1/appointments/staff/{staff_id} "
                 "[page=1,per_page=50]",
            catch_response=True,
        ) as response:
            if response.status_code != 200:
            ...
            if not isinstance(data, dict):
                response.failure(
                    "Appointment data is missing or invalid"
                )
                return
            ...
            if not isinstance(items, list):
                response.failure(
                    "Appointment items is not a list"
                )
                return

            if not isinstance(total, int):
                response.failure(
                    "Appointment total is not an integer"
                )
                return
            ...
            if not isinstance(pages, int):
                response.failure(
                    "Appointment pages is not an integer"
                )
                return

            if len(items) > 50:
                response.failure(
                    f"Returned more than 50 appointments: "
                    f"{len(items)}"
                )
                return
            ...
            for item in items:
                if not isinstance(item, dict):
                    response.failure(
                        "Appointment item is not an object"
                    )
                    return

                if item.get("staff_id") != 20:
                    response.failure(
                        "Appointment belongs to unexpected staff"
                    )
                    return
