...
class AmbulanceUser(HttpUser):
    ...
    @task
    def list_ambulance_trips(self) -> None:
        headers = {
            "Accept": "application/json",
        ...
        with self.client.get(
            ...
            headers=headers,
            name="GET /api/v1/ambulance/trips [page=1,per_page=50]",
            catch_response=True,
        ) as response:
            if response.status_code != 200:
                response.failure(
            ...
            if not isinstance(body, dict):
                response.failure(
                    "Ambulance trip response is not an object"
                )
                return

            if body.get("success") is not True:
                response.failure(
                    "Ambulance trip response success != true"
                )
                return
            ...
            if not isinstance(data, dict):
                response.failure(
                    "Ambulance trip response data is missing or invalid"
                )
                return
            ...
            if not isinstance(items, list):
                response.failure(
                    "Ambulance trip items is not a list"
                )
                return

            if not isinstance(total, int):
                response.failure(
                    "Ambulance trip total is not an integer"
                )
                return
            ...
            if len(items) > 50:
                response.failure(
                    f"Returned more than 50 ambulance trips: {len(items)}"
                )
                return
            ...
            for item in items:
                if not isinstance(item, dict):
                    response.failure(
                        "Ambulance trip item is not an object"
                    )
                    return

                if not isinstance(item.get("id"), int):
                    response.failure(
                        "Ambulance trip id is not an integer"
                    )
                    return

                if not isinstance(item.get("clinic_id"), int):
                    response.failure(
                        "Ambulance trip clinic_id is not an integer"
                    )
                    return

                if item.get("vehicle_id") is not None and not isinstance(
                    item.get("vehicle_id"),
                    int,
                ):
                    response.failure(
                        "Ambulance trip vehicle_id is not an integer"
                    )
                    return

                if item.get("status") is not None and not isinstance(
                    item.get("status"),
                    str,
                ):
                    response.failure(
                        "Ambulance trip status is not a string"
                    )
                    return
