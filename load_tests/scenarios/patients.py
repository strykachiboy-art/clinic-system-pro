...
class PatientsUser(HttpUser):
    ...
    def _login(self) -> str:
        ...
        with self.client.post(
            ...
            try:
                ...
                response.failure("HTTP 200 but response was not JSON")
                raise RuntimeError(
                    "Benchmark authentication returned invalid JSON"
                ) from exc

            if body.get("success") is not True:
                response.failure("HTTP 200 but success != true")
                raise RuntimeError("Benchmark authentication failed")

            data = body.get("data")

            if not isinstance(data, dict):
                response.failure("Login response data is missing or invalid")
                raise RuntimeError("Benchmark authentication failed")
    ...
    def list_patients(self) -> None:
        ...
        with self.client.get(
            ...
            headers=headers,
            name="GET /api/v1/patients [page=1,per_page=50]",
            catch_response=True,
        ) as response:
            if response.status_code != 200:
                response.failure(
            ...
            if not isinstance(data, dict):
                response.failure("Patient list data is missing or invalid")
                return
            ...
            if not isinstance(items, list):
                response.failure("Patient list items is not a list")
                return

            if not isinstance(total, int):
                response.failure("Patient list total is not an integer")
                return

            if page != 1:
                response.failure(f"Unexpected page value: {page}")
                return
            ...
            if len(items) > 50:
                response.failure(
                    f"Returned more than 50 patients: {len(items)}"
                )
                return
