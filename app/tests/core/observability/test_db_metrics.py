...
def test_db_metrics_counts_queries_and_records_time(
    ...
    with caplog.at_level(logging.INFO, logger=app.logger.name):
        response = client.get("/__test/observability/db")
...
def test_db_metrics_records_only_database_execution_time(
    ...
    query_count = getattr(record, "db_query_count", None)
    db_time_ms = getattr(record, "db_time_ms", None)
    request_duration_ms = getattr(record, "duration_ms", None)
    ...
    assert isinstance(request_duration_ms, (int, float))
...
def test_db_metrics_counts_multiple_queries(
    ...
    @app.get("/__test/observability/db-multiple")
    def db_multiple_queries_route():
        db.session.execute(text("SELECT 1"))
        db.session.execute(text("SELECT 2"))
        db.session.execute(text("SELECT 3"))
        db.session.execute(text("SELECT 4"))
...
def test_db_metrics_does_not_count_queries_outside_request(
    ...
    @app.get("/__test/observability/db-after-context")
...
def test_db_metrics_does_not_register_twice(
    ...
    @app.get("/__test/observability/db-duplicate")
    def db_duplicate_route():
        db.session.execute(text("SELECT 1"))
        return jsonify({"success": True})

    with caplog.at_level(logging.INFO, logger=app.logger.name):
        response = client.get("/__test/observability/db-duplicate")
